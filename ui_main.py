...
class GitManagerApp(ctk.CTk):
    ...
    def _build_icon_bar(self):
        ...
        for i,(ic,tip,active) in enumerate(icons):
            b = ctk.CTkButton(bar, text=ic, font=(self.F,22), width=48, height=48,
        ...
        ctk.CTkButton(bar, text="⚙", font=(self.F,20), width=48, height=48,
    ...
    def _build_source_panel(self):
        ...
        ctk.CTkButton(hdr, text="⋯", width=30, height=28, fg_color="transparent",
        ...
        self.repo_name_lbl = ctk.CTkLabel(bot, text="▸ 存储库  未打开仓库",
            font=(self.F,11), text_color=TEXT_PRIMARY, anchor="w")
        self.repo_name_lbl.pack(fill="x", padx=10, pady=(8,3))
        ctk.CTkLabel(bot, text="▸ 智能体审查", font=(self.F,11),
                     text_color=TEXT_PRIMARY, anchor="w").pack(fill="x", padx=10, pady=3)
        ctk.CTkLabel(bot, text="▸ 图形", font=(self.F,11),
                     text_color=TEXT_PRIMARY, anchor="w").pack(fill="x", padx=10, pady=3)
    ...
    def _build_status_bar(self):
        sb = ctk.CTkFrame(self, height=26, fg_color=BG_SIDEBAR, corner_radius=0)
        ...
        self.branch_lbl = ctk.CTkLabel(left, text="⑂ main", font=(self.F,11),
                                       text_color=TEXT_PRIMARY)
        self.branch_lbl.pack(side="left", padx=(5,10))
        ctk.CTkLabel(left, text="↻ 0↓ 0↑", font=(self.F,11),
                     text_color=TEXT_PRIMARY).pack(side="left", padx=5)
        ctk.CTkLabel(left, text="⊘ 0 ⚠ 0", font=(self.F,11),
                     text_color=TEXT_PRIMARY).pack(side="left", padx=5)
        ...
        ctk.CTkLabel(right, text="MEM 93%", font=(self.F,11),
    ...
    def _show_welcome(self):
        ...
        ctk.CTkLabel(center, text="⑂", font=(self.F,42),
                     text_color="#4FC1FF").pack(pady=(50,8))
        ctk.CTkLabel(center, text="Git Manager", font=(self.F,17,"bold"),
                     text_color=TEXT_WHITE).pack(pady=(0,3))
        ctk.CTkLabel(center, text="源代码管理工具",
                     font=(self.F,10), text_color=TEXT_DIM).pack(pady=(0,24))

        ctk.CTkFrame(center, fg_color=SEPARATOR, height=1).pack(fill="x", padx=20, pady=(0,20))

        ctk.CTkButton(center, text="📂  打开项目文件夹", fg_color=BTN_PRIMARY,
            hover_color=BTN_PRIMARY_HOVER, text_color=TEXT_WHITE,
            font=(self.F,12,"bold"), height=36, width=240, corner_radius=6,
            command=self._open_folder).pack(pady=(0,8))
        ctk.CTkLabel(center, text="选择你的项目文件夹以使用分布式版本管理系统",
            font=(self.F,9), text_color=TEXT_DIM, wraplength=260).pack(padx=20)

        ctk.CTkFrame(center, fg_color=SEPARATOR, height=1).pack(fill="x", padx=20, pady=(20,14))

        for icon, title, desc in [("⑂","版本控制","追踪文件变更"),("⊕","分支管理","并行开发"),("↻","远程同步","推拉协作")]:
            r = ctk.CTkFrame(center, fg_color="transparent", height=32)
            r.pack(fill="x", padx=16, pady=2)
            r.pack_propagate(False)
            ctk.CTkLabel(r, text=icon, font=(self.F,13),
                         text_color="#4FC1FF", width=24).pack(side="left", padx=(0,6))
            c = ctk.CTkFrame(r, fg_color="transparent")
            c.pack(side="left", fill="both", expand=True)
            ctk.CTkLabel(c, text=title, font=(self.F,10,"bold"),
                         text_color=TEXT_PRIMARY, anchor="w").pack(anchor="w")
            ctk.CTkLabel(c, text=desc, font=(self.F,9),
        ...
        ctk.CTkLabel(inner, text="Git GUI Client", font=(self.F,12),
                     text_color=TEXT_DIM).pack(pady=(0,28))

        sc = ctk.CTkFrame(inner, fg_color=BG_PANEL, corner_radius=10, width=260)
        sc.pack()
        sc.pack_propagate(False)
        ctk.CTkLabel(sc, text="快捷键", font=(self.F,10,"bold"),
                     text_color=TEXT_SECONDARY).pack(padx=14, pady=(10,4), anchor="w")
        for key, desc in [("Ctrl+Enter","提交更改"),("Ctrl+Shift+K","推送"),("Ctrl+T","同步")]:
            r = ctk.CTkFrame(sc, fg_color="transparent", height=24)
            r.pack(fill="x", padx=14, pady=1)
            r.pack_propagate(False)
            ctk.CTkLabel(r, text=key, font=(self.FM,9,"bold"),
                         text_color="#4FC1FF", width=110, anchor="w").pack(side="left")
            ctk.CTkLabel(r, text=desc, font=(self.F,9),
    ...
    def _show_not_git_repo(self, folder_name):
        ...
        ctk.CTkLabel(self.panel_content, text="当前打开的文件夹中没有 Git 存储库。\n可初始化一个仓库。它将实现 Git\n提供支持的源代码管理功能。",
            font=(self.F,12), text_color=TEXT_PRIMARY,
        ...
        ctk.CTkButton(self.panel_content, text="初始化仓库", fg_color=BG_INPUT,
            hover_color=BG_HOVER, text_color=TEXT_PRIMARY, font=(self.F,12),
            height=32, width=200, border_width=1, border_color=BORDER_COLOR,
            command=self._init_repo).pack(padx=10, pady=(0,12), anchor="w")
        ctk.CTkLabel(self.panel_content, text="若需详细了解如何在 VS Code 中使用\nGit 和源代码管理参阅我们的文档。",
            font=(self.F,11), text_color=TEXT_DIM,
        ...
        ph = ctk.CTkFrame(self.main, fg_color=BG_EDITOR, corner_radius=0)
        ph.pack(fill="both", expand=True)
        ctk.CTkLabel(ph, text="选择文件查看差异", font=(self.F,13),
    ...
    def _show_repo_view(self):
        ...
        self._collapse_btn = ctk.CTkButton(ch, text="▾ 更改", width=80, height=28,
            fg_color="transparent", hover_color=BG_HOVER, text_color=TEXT_PRIMARY,
            anchor="w", font=(self.F,12), command=self._toggle_files)
        ...
        for txt, cmd in [("✓",self._stage_all),("↻",self._refresh),("⋯",self._show_more_menu)]:
            ctk.CTkButton(ops, text=txt, width=24, height=24, fg_color="transparent",
        ...
        self.commit_entry = ctk.CTkEntry(ci, placeholder_text="提交变更内容...",
            ...
            font=(self.F,12), corner_radius=4)
        ...
        ctk.CTkButton(ci, text="✦", width=28, height=28, fg_color=BG_INPUT,
        ...
        self.commit_btn = ctk.CTkButton(cb, text="✓ 提交 Ctrl+Enter", fg_color=BTN_PRIMARY,
            hover_color=BTN_PRIMARY_HOVER, text_color=TEXT_WHITE,
            font=(self.F,12,"bold"), height=30, command=self._commit)
        self.commit_btn.pack(side="left", fill="x", expand=True, padx=(0,3))
        ctk.CTkButton(cb, text="▾", width=28, height=30, fg_color=BTN_PRIMARY,
            hover_color=BTN_PRIMARY_HOVER, text_color=TEXT_WHITE,
    ...
    def _show_diff_view(self, f: GitFile):
        for w in self.main.winfo_children(): w.destroy()
        tb = ctk.CTkFrame(self.main, fg_color=BG_PANEL, height=36, corner_radius=0)
        tb.pack(fill="x")
        tb.pack_propagate(False)
        ctk.CTkLabel(tb, text=f"  {f.path}  ({f.status_text})",
                     font=(self.FM,12), text_color=TEXT_PRIMARY, anchor="w").pack(side="left", padx=5)
        ctk.CTkButton(tb, text="×", width=30, height=26, fg_color="transparent",
            hover_color=BG_HOVER, text_color=TEXT_SECONDARY,
    ...
    def _refresh(self):
        ...
        if staged:
            hdr = ctk.CTkFrame(self.files_frame, fg_color="transparent", height=26)
            hdr.grid(row=row, column=0, sticky="ew", padx=5)
            hdr.grid_propagate(False)
            ctk.CTkLabel(hdr, text=f"▾ 暂存的更改  {len(staged)}",
                font=(self.F,11), text_color=TEXT_PRIMARY, anchor="w").pack(side="left")
            ctk.CTkButton(hdr, text="−", width=22, height=20, fg_color="transparent",
                hover_color=BG_HOVER, text_color=TEXT_SECONDARY,
        ...
        if not files:
            ctk.CTkLabel(self.files_frame, text="没有更改", font=(self.F,12),
    ...
    def _file_row(self, f: GitFile, row: int, staged: bool):
        rf = ctk.CTkFrame(self.files_frame, fg_color="transparent", height=30)
        ...
        if os.path.exists(icon_path):
            ...
            icon = ctk.CTkImage(light_image=Image.open(icon_path), size=(18, 18))
            ctk.CTkLabel(rf, image=icon, text="", width=18, height=18).grid(
                row=0, column=0, padx=(5,6), pady=5)
        else:
            ctk.CTkLabel(rf, text="··", font=(self.FM,9), text_color="#666",
                         width=18).grid(row=0, column=0, padx=(5,6), pady=5)

        # 文件名
        name = f.path if len(f.path) <= 42 else "..." + f.path[-39:]
        ctk.CTkLabel(rf, text=name, font=(self.F,11), text_color=TEXT_PRIMARY,
                     anchor="w").grid(row=0, column=1, sticky="w", padx=2)

        # 操作按钮
        btns = ctk.CTkFrame(rf, fg_color="transparent")
        btns.grid(row=0, column=2, padx=2)
        ctk.CTkButton(btns, text="◫", width=20, height=20, fg_color="transparent",
            hover_color=BG_HOVER, text_color=TEXT_SECONDARY, font=(self.F,11),
            command=lambda p=f.path: self.git.open_file(p)).pack(side="left", padx=1)
        if staged:
            ctk.CTkButton(btns, text="−", width=20, height=20, fg_color="transparent",
                hover_color=BG_HOVER, text_color=TEXT_SECONDARY, font=(self.F,12),
        ...
        ctk.CTkLabel(rf, text=f.status, font=(self.FM,9,"bold"),
    ...
    def _dialog(self, title, fields, cb):
        ...
        for i,(label,default) in enumerate(fields):
            ctk.CTkLabel(d, text=label, font=(self.F,12),
                         text_color=TEXT_PRIMARY).pack(padx=20, pady=(15 if i==0 else 5,2), anchor="w")
            e = ctk.CTkEntry(d, width=370, height=30, fg_color=BG_INPUT,
                text_color=TEXT_PRIMARY, border_color=BORDER_COLOR, font=(self.F,12))
        ...
        ctk.CTkButton(bf, text="确定", width=100, height=30, fg_color=BTN_PRIMARY,
            hover_color=BTN_PRIMARY_HOVER, text_color=TEXT_WHITE, command=ok).pack(side="left", padx=5)
        ctk.CTkButton(bf, text="取消", width=100, height=30, fg_color=BG_INPUT,
